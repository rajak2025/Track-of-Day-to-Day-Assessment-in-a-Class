...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 if (empty($_POST["oopj"])) {
    ...
    if (empty($_POST["vee"])) {
    $veeErr = " Mandatory field";
  } else {
    $vee = processText($_POST["vee"]);
    }
    if ($oopjErr==""&&$coErr==""&&$osErr==""&&$veeErr=="") {
      # code...
           //connecting to the database
      $connect=mysqli_connect("localhost","root","","schedule") or die("nope");
      $sqll=mysqli_query($connect,"insert into cseat(oopj,co,os,vee,date) values('$oopj','$co','$os','$vee','$date')");
         exit();
          echo "submitted!";
...
<a href="logout.php" class="button">Logout</a>
  ...
  <tr>
    ...
    <th>OS<span class="error">* <br><?php echo $osErr;?></span></th>
     ...
     <th>VEE<span class="error">* <br><?php echo $veeErr;?></span></th>
  </tr>
  <tr>
    <td><textarea  name="oopj" placeholder="Enter what was taught in oopj class" rows="5" cols="40" id="OOPJbox"></textarea></td>
    
    <td><textarea  name="os" placeholder="Enter what was taught in os class" rows="5" cols="40" id="cotbox"></textarea></td>
     <td rowspan="6"align="center"><strong>L<BR>U<BR>N<BR>C<BR>H<BR></strong></td>
    <td><textarea  name="co" placeholder="Enter what was taught in co class" rows="5" cols="40" id="osbox"></textarea></td>
    
    <td><textarea name="vee" placeholder="Enter what was taught in vee class" rows="5" cols="40" id="flatbox"></textarea></td>
